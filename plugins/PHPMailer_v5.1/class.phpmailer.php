...
// There should not be any EOL in the string
    ...
    switch (strtolower($position)) {
      case 'phrase':
        $encoded = preg_replace_callback("|([^A-Za-z0-9!*+\/ -])|", function($found) {
          return strtolower($found[1]);
        }, $encoded);
        break;
      case 'comment':
        $encoded = preg_replace_callback("/([\(\)\"])/", function($found) {
          return strtolower($found[1]);
        }, $encoded);
      case 'text':
      default:
// Replace every high ascii, control =, ? and _ characters
//TODO using /e (equivalent to eval()) is probably not a good idea
        $encoded = preg_replace_callback('|([\000-\011\013\014\016-\037\075\077\137\177-\377])|', function($found) {
          return strtolower($found[1]);
        }, $encoded);
        break;
    }
