...
class retorno {
    ...
    static function json($call, $all = true) {
        if ($all)
...
class Dml {
    ...
    static function getLastId($conexao, $genarator) {
    ...
    static function obr($conexao, $param) {

        /* get campos not null do bando de dados */
        $sql = 'SELECT RDB$FIELD_NAME FIELDS
              FROM RDB$RELATION_FIELDS A
              WHERE (A.RDB$RELATION_NAME = ' . "'" . $this->nomeTabela . "'" . ')AND (A.RDB$NULL_FLAG = 1)';
        $pdo = $conexao->prepare($sql);
        $pdo->execute();
        ...
        $sql = 'SELECT idx.RDB$FIELD_NAME FIELDS
              ...
              AND tc.RDB$RELATION_NAME = ' . "'" . $this->nomeTabela . "'" . '
...
//                    die('{"erro":"Inexistência de parâmentos"}');
    ...
    static function insert($conexao, $tabela, $arrayCampos, $printSQL = false) {
...
//        unset($arrayCampos['id_sociedade']);
    ...
    static function insertUpdateMySQL($conexao, $tabela, $arrayCampos, $printSQL = false) {
        $sql = "INSERT INTO $tabela ";
    ...
    static function update($conexao, $tabela, $arrayCampos, $where_and) {
        unset($arrayCampos['ax']);
        unset($arrayCampos['id_sociedade']);
    ...
    static function duplicity($conexao, $param) {
...
//    echo json_encode($rs);
    ...
    static function getRegistro($conexao, $campos, $tabela, $campoFiltro = '', $id = '', $orderBy = '') {
