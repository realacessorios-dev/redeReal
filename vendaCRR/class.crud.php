...
class crud
{
    ...
    function getVendaVendedor($param)
    {
        ...
        foreach ($param['cpfs'] as $value) {

            $cpfs .= "'" . $value . "', ";
        }

        $cpfs = substr($cpfs, 0, -2);


        $sql = "select  DISTINCT(B.LOGIN), a.ID_VENDEDOR, C.descricao AS Cargo,
                SUM(A.valor + a.valor_montagen) AS VALOR, A.mes, A.ano, B.CPF
                ...
                and b.cpf in($cpfs)
                group BY B.LOGIN, C.DESCRICAO, a.id_vendedor, A.MES, A.ANO, B.CPF
                ORDER BY B.LOGIN";
    ...
    function getFuncionario()
    {
        ...
        $sql = "select cpf, login, nome_comp from funcionario
                where deletado = 'N'
                order by nome_comp";
